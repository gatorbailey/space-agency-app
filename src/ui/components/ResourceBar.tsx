import { useGame } from '../useGame'

function formatBudget(value: number): string {
  return `$${Math.round(value).toLocaleString()}`
}

export function ResourceBar() {
  const { state, dispatch } = useGame()
  const { resources, pendingMaterials, facility } = state

  return (
    <div className="flex flex-wrap items-center gap-4 border-b border-slate-800 bg-slate-900 px-4 py-3 text-sm text-slate-200">
      <Stat label="Sentiment" value={`${Math.round(resources.sentiment)}`} accent="text-sky-300" />
      <Stat label="Budget" value={formatBudget(resources.budget)} accent="text-emerald-300" />
      <Stat label="Materials" value={`${Math.round(resources.materials)}`} accent="text-amber-300" />
      <Stat label="Crew Readiness" value={`${Math.round(resources.crewReadiness)}`} accent="text-violet-300" />

      <div className="ml-auto flex items-center gap-2">
        <span className="text-xs text-slate-400">
          Materials pending: {Math.round(pendingMaterials)}/{facility.materialsStorageCap}
        </span>
        <button
          type="button"
          disabled={pendingMaterials <= 0}
          onClick={() => dispatch({ type: 'COLLECT_MATERIALS' })}
          className="rounded bg-amber-500 px-3 py-1 text-xs font-semibold text-slate-950 transition disabled:cursor-not-allowed disabled:opacity-40"
        >
          Collect
        </button>
      </div>
    </div>
  )
}

function Stat({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <div className="flex items-baseline gap-1.5">
      <span className="text-xs uppercase tracking-wide text-slate-500">{label}</span>
      <span className={`font-mono text-base font-semibold ${accent}`}>{value}</span>
    </div>
  )
}
